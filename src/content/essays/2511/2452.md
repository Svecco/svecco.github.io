...
ethos, manages hundreds of influential **open source** projects that underpin
...
[^apache]: _**The Apache Software Foundation**_. _"Apache Software Foundation [**Celebrates 25 Years**](https://news.apache.org/foundation/entry/apache-software-foundation-celebrates-25-years)_."
(**Mar 25, 2024**). _"Valuating Code at The Apache Software Foundation"_, 
[_**DinoSource**_](https://apache.org/foundation/docs/Valuating_Code_at_the_ASF.pdf), (**2022**), cited on [Feb 13, 2026]
...
:::note[When Editing]
The **syntax** has been **corrected** much by **LLM**; This article was
written on **February 13, 2026,** after 
...
, so I marked it published on **November 30.**
:::
...
In **2021**, I learned about [_**Log4j**_](https://www.cisa.gov/news-events/news/apache-log4j-vulnerability-guidance)
([_**CVE-2021-44228**_](https://nvd.nist.gov/vuln/detail/CVE-2021-44228))
`CVSSv3 10.0/10.0` vulnerability[^CVE-2021-44228].      
_I'm sorry to have gotten to know you this way._

[^CVE-2021-44228]: _**America's Cyber Defense Agency**_, _"Apache Log4j Vulnerability Guidance"_,
_**ED 22-02**_. (Apr 08, 2022) [Feb 13, 2026]
Also _**Log4Shell**_. It is a [RCE](https://www.crowdstrike.com/en-us/cybersecurity-101/cyberattacks/remote-code-execution/) 
vulnerability in [_**Apache Log4j**_](https://logging.apache.org/log4j/) 
logging library disclosed on **December 9, 2021**. `CVSSv3 10.0/10.0` is the
**highest** severity level, and is widely described as one of 
the most severe cybersecurity vulnerabilities in recent years.

## The Apache Iggy

This brings me to **the situation** I had in **early November 2025**.

**First**, while I already knew how to use **proxies**[^proxy]
in the _5th_ grade, 
...
completely **squandered**[^eleh] most of my time that could have been spent 
tinkering with distros and coding langs[^addicted].

[^proxy]: The days I was still using `shadowsocks` on 
[_**Windows 7**_](https://en.wikipedia.org/wiki/Windows_7).

[^jv]: _**Gardner J**_. [_"Why Java Sucks"_](https://tech.jonathangardner.net/wiki/Why_Java_Sucks)
. (Jul 18, 2017)[Feb 14, 2026].

[^eleh]: Yes, I *truly* think this. I've lost interests in games.

[^addicted]: At the worst point during this period, I *forgot* the name
of the *PCIe* interface. I don't even know how to use 
[_**Tmux**_](https://en.wikipedia.org/wiki/Tmux). 
I truly admire *some current middle*
school students who can _proficiently participate_ in coding
and _actively contribute to_ various projects _but as a high
school stu I could not_. It was not until
*late 2023 to early 2025* that I gradually got back on track
through [Ubuntu](https://ubuntu.com/).
...
4. The project **language** I prefer to use[^rust].

[^rust]: Usually I'd like Rust now. *Elegance*, performance, and safety.
...
| **Path**                           | **Work**                     |
|------------------------------------|------------------------------|
| core/server/src/**binary**         | *Binary protocol processing* |
| core/server/src/**compat**         | *Compatibility layer*        |
| <u>core/server/src/**configs**</u> | <u>*Config management*</u>   |
| core/server/src/**http**           | *HTTP API impl*              |
| core/server/src/**io**             | *IO operations*              |
| <u>core/server/src/**log**</u>     | <u>*Logging management*</u>  |
| core/server/src/**metadata**       | *Metadata management*        |
| core/server/src/**quic**           | *QUIC protocol impl*         |
| core/server/src/**shard**          | *Shard execution unit*       |
| core/server/src/**state**          | *System state persistence*   |
| core/server/src/**streaming**      | *Core stream processing*     |
| core/server/src/**tcp**            | *TCP protocol impl*          |
| core/server/src/**websocket**      | *WebSocket support*          |

_And for the issue, for example_, [server.toml](https://github.com/apache/iggy/blob/master/core/server/config.toml) 
controls the default[^renamed] configuration file, while
...
[^renamed]: We renamed `server.toml` to `config.toml` and
imported breaking changes later.
...
I think current _LLMs_ are still quite capable when it comes to
_summarizing projects' structure_ and _extracting macros/functions usage_.
Why fight against productivity, right.[^cite]

[^cite]: D.C., _**SaTCaPPF**_ (1988) [Feb 14, 2026]

:::note[About My LLMs Usage]
They can quickly _locate the target_ and explain it clearly,
even though *current models* aren’t very good at writing
code when the *prompt* isn’t highly detailed. Blindly
relying on LLMs, though this may not be the case in
the future, would be overestimating how intelligent they are right now.
But it's still quite convenient for **trace** or **call tree** usage, that's pretty good.
:::

![justsignal.webp](rc/justsignal.webp)
...
[^amd_smt]: _**Kotsiolis**, **Apostolos**_. _"[Simultaneous Multithreading](https://www.amd.com/en/blogs/2025/simultaneous-multithreading-driving-performance-a.html): Driving Performance and Efficiency on AMD EPYC CPUs"_
. AMD Official Blog, (March 3, 2025) [Feb 14, 2026]
...
check out the _**VCR**_,[^vcr] and, find there may something incorrect, but you don't know exactly what it is:

[^vcr]: A Chinese internet *meme*, often used when arguing against someone.
...
[^doc]: **_Rust Team_**. [_**The Rust Standard Library**_](https://doc.rust-lang.org/stable/std/index.html) 
`std`. `01f6ddf75` (Feb 11, 2026)[Feb 14, 2026]

"_**NUMA** regions_"? What is "_**NUMA**_"?

In early computer systems, all `CPU` accessed memory through a single
bus, an architecture known as **_SMP_**. 
...
[^hardware_loc]: _**Wikipedia Contributors**_.
[_**Non-uniform memory access**_](https://en.wikipedia.org/wiki/Non-uniform_memory_access)
. _Wikipedia, The Free Encyclopedia_. (2026) [Feb 14, 2026]
...
[^zhihu]: User[@linux](https://www.zhihu.com/people/ning-meng-meng-meng-47-83)
, _"What is NUMA, and [why](https://zhuanlan.zhihu.com/p/643610982) do we need to understand NUMA?"_. 
Zhihu, Linux Server Development Column. (July 14, 2023) [Feb 14, 2026]
...
[^hwlocality]: [_**hwlocality**_](https://docs.rs/hwlocality/latest/hwlocality/)
is a _*Rust*_ binding for `hwloc`, designed for _**hardware topology detection**_, 
_**NUMA**_ support, and _**thread binding optimization**_. It's secure, 
easy to use & actively maintained.

## Wait, Some Compiling Dependencies Missing

It turns out that `hwloc-devel` was missing, maybe can infer that 
the bug has truly been fixed()[^parentheses]

[^parentheses]: I don't know why that it seems like _empty parentheses_
like this can *alleviate* the awkwardness nowadays on the internet?
...
But **IT IS WRONG**[^iec]. The correct one is following 
[_**IEC 60027-2**_](https://en.wikipedia.org/wiki/IEC_60027), which looks 
[like this](https://www.computernetworkingnotes.com/networking-tutorials/megabytes-mb-v-s-mebibytes-mib-differences-explained.html).
...
[^iec]: [_**SI**_](https://www.iec.ch/si)
, by [_**CGPM**_](https://www.bipm.org/en/committees/cg/cgpm)
, <u>_Conférence Générale des Poids et Measures_</u>.
[_**IEC 60027**_](https://en.wikipedia.org/wiki/International_Electrotechnical_Commission)
, by [_**IEC**_](https://www.iec.ch)
, <u>_International Electrotechnical Commission_.</u>
**Wikimedia Foundation, Inc**; **International Electrotechnical 
Commission**. Wikipedia, (November 2023) [Feb 14, 2026]
...
[^the_book]: **Jim Blandy**, **Jason Orendorff**, **Leonora F. S. Tindall**. 
"[_Programming Rust 2nd Edition_](https://www.oreilly.com/library/view/programming-rust-2nd/9781492052586/)
: _Fast, Safe Systems Development_" Sebastopol, 
CA: _**O'Reilly Media Inc.**_(2021)[Feb 14, 2026]
...
```log title="fish $ cargo build --all-targets --release --workspace --all-features" collapse={2-7}
warning: Invalid record (Producer: 'LLVM21.1.3-rust-1.92.0-stable' Reader: 'LLVM21.1.3-rust-1.92.0-stable')
...
[^ci_billing]: _**GitHub Inc**_.
[_**"GitHub Actions billing."**_](https://docs.github.com/en/billing/concepts/product-billing/github-actions)
GitHub Docs, (Jan 13 2026) [Feb 14, 2026]
...
[^sponsor_ship] but what matters more is this line from **@spetz** after I asked:

> _**Thank you for all the changes, and dont worry about CI.**_

[^sponsors]: _**The Apache Software Foundation**_. "[_Our Sponsors_](https://www.apache.org/foundation/sponsors)"
_Apache Software Foundation Website_. Sponsors, community, projects, and brand information. (2025) [Feb 14, 2026]

[^sponsor_ship]: **_The Apache Software Foundation_**. “[_Our Sponsorship Program._](https://www.apache.org/foundation/sponsorship.html)”
_Apache Software Foundation Website_. Sponsors, community, projects, and brand information. (2025) [Feb 14, 2026]
...
[^runners]: _**Act**_, ["_Runners_"](https://nektosact.com/usage/runners.html),
Act User Guide, Manual, Docs, Documentation. (2026) [Feb 14, 2026]
...
[^llama_one]: _**Llama 4 Scout**_ Generated. "_What is 'DinD'?_" 
_**Meta Inc.**_ (Feb 14, 2025)[Feb 14, 2025]

[^llama_two]: _**Llama 4 Scout**_ Generated. "_What is quickwit-sink?_"
_**Meta Inc.**_ (Feb 14, 2025)[Feb 14, 2025]

:::important[You May Have to Generate a _**GitHub Token**_]
When `act` pulls third party Actions from _**GitHub**_ Marketplace,
such as `codecov-action`, a _**GitHub Token**_ is required for `API`
authentication to _avoid rate limiting or access denial_.
[_**Teleport**_](https://github.com/settings/tokens).
You can use `--secret-file <path/to/token/file>` to pass the token to `act`.
:::
...
```log title="My Honey."
────────────
     Summary [ 209.765s] 1313 tests run: 1313 passed (5 slow), 96 skipped
Notice: Tests executed in 59s (00:59)

=========================================
All targets build:             21s (00:21)
Tests compile:                 39s (00:39)
Tests execute:                 59s (00:59)
-----------------------------------------
Total build:                   60s (01:00)
Total time:                    209s (03:29)
=========================================
```
...
> 
> *This is my first ever Rust PR, and also my first contribution to be*
> *merged into a formal project that's nearing 4k stars, rather than a*
> *package repo or other casual repositories. As a student, I initially*
> *feared the long dev and review cycle would be a hassle, but I’m*
> *extremely grateful to*
> [_**@hubcio**_](https://github.com/hubcio)
> *and*
> [_**@spetz**_](https://github.com/spetz)
> *for your unparalleled*
> *patience in guiding and reviewing my work, who have helped me*
> *get up to speed with the workflow of a production ready backend*
> *project. I’ve also learned many things else e.g.podman, and*
> *many exp that I'd never get in school.*
> 
> *Thanks, again :)* [^github_comments]

[^github_comments]: _**Chenrui Wu**_, [Code 3858794990 Comments](https://github.com/apache/iggy/pull/2452#issuecomment-3858794990)
, Free, (Feb 6, 2025)[Feb 6, 2025]