...
Ah, if `hwloc.pc` is in the `dev` of the `***nixpkgs***`, there will be multiple outputs...
...
Since the issue can only be reproduced on `***NixOS***` :spoiler[... ...] and cannot be reproduced on `***Fedora***` or `***Gentoo***`
...
`Act` uses `***Docker***` to run the `CI/CD` workflow, which is the default,
...
When `act` runs third-party Actions from the GitHub Marketplace (e.g., `***codecov-action***`).
